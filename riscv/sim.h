...
public:
  ...
  abstract_interrupt_controller_t* get_intctrl() const {
    if (aplic_s) return aplic_s.get();
    assert(plic.get()); return plic.get();
  }
...
private:
  ...
  void interactive_virt(const std::string& cmd, const std::vector<std::string>& args);
  ...
  void interactive_dev(const std::string& cmd, const std::vector<std::string>& args);
