...
bool hstatus_csr_t::unlogged_write(const reg_t val) noexcept {
  const reg_t mask = HSTATUS_VTSR | HSTATUS_VTW
    ...
    | HSTATUS_HU | HSTATUS_SPVP | HSTATUS_SPV | HSTATUS_GVA | (proc->extension_enabled_const(EXT_SSAIA) ? HSTATUS_VGEIN : 0);
  ...
  if (proc->extension_enabled_const(EXT_SSAIA)) {
    ...
    // if vgein = 0, clear VSEIP; otherwise IMSIC updates it
    if (new_vgein == 0)
      state->mip->backdoor_write_with_mask(MIP_VSEIP, 0);
    else if (new_vgein != old_vgein)
